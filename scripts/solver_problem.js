...
function drawCeresProblem(){
  ...
  const equality=selectedMode.settings.length_equality_prior;
  const proportion=selectedMode.settings.length_proportion_prior;
  ...
  for(const index of indices){
    ...
    if(equality?.enabled){
      residuals.push({index,bodyIndex:equality.segments[0],bodyIndices:equality.segments,kind:'length_equality',bodyLabel:'Sacrolumbar / thoracic lengths',name:'LengthEqualityResidual',
        connections:equality.segments.map(b=>`length-${index}-${b}`),detail:`1 component; scale ${equality.scale_mm} mm; within-frame preference, not temporal smoothing or a fixed total length`,formula:'r = sqrt(time weight) * (length A - length B) / scale'});
    }
    if(proportion?.enabled){
      residuals.push({index,bodyIndex:proportion.segments[0],bodyIndices:proportion.segments,kind:'length_proportion',bodyLabel:'Three spine lengths',name:'LengthProportionResidual',
        connections:proportion.segments.map(b=>`length-${index}-${b}`),detail:`3 components (rank 2); ${proportion.segment_names.join(' : ')} = ${proportion.ratios.join(' : ')}; scale ${proportion.scale_mm} mm; total length free`,formula:'r_i = sqrt(time weight) * (length_i - fraction_i * sum(lengths)) / scale'});
    }
