...
from skellyforge.skellymodels.utils.types import BoneKey, BoneLengthRatios, MarkerName, SegmentName, SegmentConnection, SegmentCenterOfMassDefinition
...
class ModelInfo(BaseModel):
    ...
    def tracked_point_slices(self) -> Dict[str, slice]:
        """Per-aspect slices into the concatenated tracked-point array."""
        slices = {}
        current_marker = 0
        for aspect in self.order:
            try:
                num_tracked_points = self.aspects[aspect].num_tracked_points
                slices[aspect] = slice(current_marker, current_marker + num_tracked_points)
                current_marker += num_tracked_points
            except KeyError:
