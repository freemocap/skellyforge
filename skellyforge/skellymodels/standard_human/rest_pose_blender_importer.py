...
def _build_landmark_empties(document: dict) -> None:
    ...
    mirrored = _get_or_make_collection(_MIRRORED_COLLECTION)
...
def load_rest_pose(json_path: str = JSON_PATH) -> None:
    ...
    if bpy.context.object is not None and bpy.context.object.mode != "OBJECT":
        bpy.ops.object.mode_set(mode="OBJECT")
    _clear_previous()
    _build_armature(document)
    _build_landmark_empties(document)
    counts = document.get("counts", {})
    print(
        f"[standard_human] loaded {counts.get('bones', '?')} bones, "
        f"{counts.get('landmarks', '?')} landmarks from {json_path}"
    )
