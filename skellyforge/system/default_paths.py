...
DEFAULT_SKELLYFORGE_BASE_FOLDER_NAME = "skellyforge_data"
...
SKELLYFORGE_SVG_PATH = str(
    Path(__file__).parent.parent.parent / "shared" / "skellyforge-logo" / "skellyforge-logo.svg")
SKELLYFORGE_FAVICON_ICO_PATH = str(
    Path(__file__).parent.parent.parent / "shared" / "skellyforge-logo" / "skellyforge-favicon.ico")
...
def get_default_skellyforge_base_folder_path() -> str:
    return str(Path(os_independent_home_dir()) / DEFAULT_SKELLYFORGE_BASE_FOLDER_NAME)
...
def get_log_file_path() -> str:
    log_folder_path = (
            Path(get_default_skellyforge_base_folder_path()) / LOGS_INFO_AND_SETTINGS_FOLDER_NAME / LOG_FILE_FOLDER_NAME
    )
